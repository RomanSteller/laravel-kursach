<template>
    <div v-for="(message,index) in this.messages" :key="index">
        <div v-if="message.users.id === this.$store.getters.setUser.id" class="message my-message">
        ...
        <div v-else-if="message.users.id !== this.$store.getters.setUser.id" class="message info">
            <img alt="" class="img-circle medium-image" src="https://bootdey.com/img/Content/avatar/avatar1.png">
            <div class="message-body">

                <div class="message-info"><h4> {{ message.users.login }}</h4></div>
                <hr>
...
<script>
export default {
    name: "renderFromServer-component",
    props: ['messages'],
    mounted() {
        console.log('сука нахуй')
        console.log('dasdasd' + this.messages)
    }
}
</script>
